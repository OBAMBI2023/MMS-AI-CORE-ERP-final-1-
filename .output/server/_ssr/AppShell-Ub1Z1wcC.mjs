...
import { B as Menu, L as Moon, U as LogOut, _ as Sparkles, _t as ChevronDown, h as Sun, o as User, w as Settings } from "../_libs/lucide-react.mjs";
...
import { n as useTheme } from "./theme-provider-DGWgppJh.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/AppShell-Ub1Z1wcC.js
...
function UserMenu() {
	...
	const { data: profile } = useQuery({
		...
		queryFn: async () => {
			...
			if (!user) return null;
			const { data, error } = await supabase.from("profiles").select("full_name, roles(name)").eq("id", user.id).single();
