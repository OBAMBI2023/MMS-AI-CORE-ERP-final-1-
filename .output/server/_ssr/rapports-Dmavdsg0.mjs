...
import { a as Users, h as Sun, i as Wallet, st as Download, tt as FileText, y as ShoppingCart, yt as ChartColumn } from "../_libs/lucide-react.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/rapports-Dmavdsg0.js
...
function RapportsPage() {
	...
	const exportPDF = async () => {
		...
		doc.setFontSize(12);
		doc.text(`Rapport du mois de ${new Date(year, month - 1).toLocaleString("fr-FR", { month: "long" })} ${year}`, 50, 30);
		const summaryData = [
		...
		let detailedBody = [];
		const filtered = {
			ventes: data.ventes.filter((v) => new Date(v.created_at).getMonth() + 1 === month && new Date(v.created_at).getFullYear() === year),
			depenses: data.depenses.filter((d) => new Date(d.paid_at).getMonth() + 1 === month && new Date(d.paid_at).getFullYear() === year),
			achats: data.achats.filter((a) => new Date(a.created_at).getMonth() + 1 === month && new Date(a.created_at).getFullYear() === year),
			devis: data.devis.filter((d) => new Date(d.created_at).getMonth() + 1 === month && new Date(d.created_at).getFullYear() === year),
			clients: data.clients.filter((c) => new Date(c.created_at).getMonth() + 1 === month && new Date(c.created_at).getFullYear() === year)
		};
		filtered.ventes.forEach((v) => detailedBody.push([
			"Vente",
			new Date(v.created_at).toLocaleDateString("fr-FR"),
			formatCurrency(Number(v.total))
		]));
		filtered.achats.forEach((a) => detailedBody.push([
			"Achat",
			new Date(a.created_at).toLocaleDateString("fr-FR"),
			formatCurrency(Number(a.total))
		]));
		filtered.depenses.forEach((d) => detailedBody.push([
			"Dépense",
			new Date(d.paid_at).toLocaleDateString("fr-FR"),
			formatCurrency(Number(d.amount))
		]));
		filtered.devis.forEach((d) => detailedBody.push([
			"Devis",
			new Date(d.created_at).toLocaleDateString("fr-FR"),
			formatCurrency(Number(d.total))
		]));
		filtered.clients.forEach((c) => detailedBody.push([
			"Client",
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(AppShell, {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				}), canExport && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DropdownMenu, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DropdownMenuTrigger, {
					asChild: true,
...
function StatCard({ label, value, compare, icon: Icon, color }) {
	...
	isCurrency ? formatCurrency(Math.abs(compare.diff)) : Math.abs(compare.diff);
