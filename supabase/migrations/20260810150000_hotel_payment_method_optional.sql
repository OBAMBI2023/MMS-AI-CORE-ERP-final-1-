-- Payment method becomes a traceability detail, not a condition to record a
-- payment: some tenants collect cash/transfers without a formal method label
-- at the front desk. hotel_reservation_payments.method was NOT NULL and the
-- collect_hotel_invoice_payment RPC raised when it was blank. Drop the NOT
-- NULL constraint and the "obligatoire" check, and normalize blank input to
-- NULL on insert (was already doing this for reference/notes). Amount and
-- paid_at validation, balance checks, tenant isolation and permissions are
-- unchanged.
