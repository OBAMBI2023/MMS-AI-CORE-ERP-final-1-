-- Manager (HOTEL) is missing hotel.reservations.check_in / check_out.
-- initialize_tenant_roles() grants Gerant these two permissions but omits
-- them from Manager's array, so every HOTEL tenant's Manager role can see
-- the Arrivee/Depart buttons (view/update perms are present) but the
-- hotel_reservations status-change trigger rejects the mutation with
-- "Permission check-in requise" / "Permission check-out requise".
-- Comptable intentionally keeps neither permission (view-only), unchanged.

-- 1) Backfill existing HOTEL tenants' Manager role (idempotent).
INSERT INTO public.role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM public.roles r
JOIN public.tenants t ON t.id = r.tenant_id
JOIN public.permissions p ON p.code IN ('hotel.reservations.check_in', 'hotel.reservations.check_out')
WHERE t.platform_type = 'HOTEL'
  AND r.name = 'Manager'
ON CONFLICT (role_id, permission_id) DO NOTHING;

-- 2) Forward fix: every future HOTEL tenant's Manager role gets these
-- permissions automatically through initialize_tenant_roles().
...
BEGIN
  ...
  LOOP
    ...
    ELSIF target_platform_type = 'HOTEL' AND default_role.name IN ('Manager', 'Gérant', 'Comptable') THEN
      ...
      -- these roles on a fresh tenant — see 20260816120000's header comment.
      -- Grant the reference HOTEL permission set directly instead.
      INSERT INTO public.role_permissions (role_id, permission_id)
      SELECT created_role_id, permission.id
      FROM public.permissions permission
      WHERE permission.code = ANY(
        CASE default_role.name
          WHEN 'Manager' THEN ARRAY[
            ...
            'hotel.reports.view',
            'hotel.reservations.check_in','hotel.reservations.check_out','hotel.reservations.create','hotel.reservations.update','hotel.reservations.view',
  ...
  -- Caissier et Commercial doivent toujours pouvoir vendre, indépendamment du
  -- résultat de la copie par template ci-dessus.
  INSERT INTO public.role_permissions (role_id, permission_id)
