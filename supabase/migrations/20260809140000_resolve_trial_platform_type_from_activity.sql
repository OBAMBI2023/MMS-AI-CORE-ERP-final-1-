-- The public /essai-gratuit sector dropdown was simplified to 6 grouped
-- options (see src/lib/trial-activities.ts): "Hôtel / Résidence /
-- Hébergement" now carries the single code "hotel". create_trial_workspace
-- previously hardcoded resolved_platform_type to 'ERP' for every activity
-- (20260805200000_fix_trial_signup_pack_status_duration.sql). This migration
-- only changes that one resolution: activity = 'hotel' now provisions
-- platform_type = 'HOTEL', every other activity keeps provisioning 'ERP'.
-- Everything else (module pack assignment, trial duration, signup flow) is
-- unchanged.
