-- Le Comptable HOTEL doit pouvoir consulter l'annuaire des Prestataires
-- (hotel.maintenance.view) en lecture seule, utile pour rapprocher une
-- dépense/facture avec le prestataire correspondant. Il ne doit pas pouvoir
-- créer/modifier/supprimer (pas de hotel.maintenance.create/update).

-- 1) Backfill des tenants HOTEL existants (idempotent).
INSERT INTO public.role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM public.roles r
JOIN public.tenants t ON t.id = r.tenant_id
JOIN public.permissions p ON p.code = 'hotel.maintenance.view'
WHERE t.platform_type = 'HOTEL'
  AND r.name = 'Comptable'
ON CONFLICT (role_id, permission_id) DO NOTHING;

-- 2) Forward fix: étend le tableau de permissions Comptable dans
-- initialize_tenant_roles() pour que chaque futur tenant HOTEL l'obtienne
-- directement, sans étape manuelle.
