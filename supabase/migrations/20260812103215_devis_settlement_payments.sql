...
AS $$
  ...
  SELECT
    ...
    COALESCE((SELECT SUM(d.amount) FROM public.depenses d, tid WHERE d.tenant_id = tid.t AND d.created_at >= m.month_start AND d.created_at < m.month_end), 0) AS depenses,
