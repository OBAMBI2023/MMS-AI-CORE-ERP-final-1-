...
-- updated_at housekeeping for direct updates (e.g. super-admin status/priority
-- changes via the service-role client) — reuses the project-wide trigger fn.
DROP TRIGGER IF EXISTS support_tickets_set_updated_at ON public.support_tickets;
CREATE TRIGGER support_tickets_set_updated_at
BEFORE UPDATE ON public.support_tickets
FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

-- Every new message bumps the ticket's last_message_at/updated_at, and
-- encodes the ticket status auto-transitions ("business rules"): a tenant
-- reply on a resolved/closed ticket reopens it; the support team's first
-- reply on an open ticket marks it in_progress. Runs for both the tenant
-- RPC path and the super-admin service-role insert path, since it's a plain
-- AFTER INSERT trigger on the table itself.
...
-- Read-only for `authenticated` (own tenant + view permission, or platform
-- admin). No INSERT/UPDATE/DELETE policy is granted to `authenticated` at
-- all — every write goes through a SECURITY DEFINER RPC below, which runs as
-- the function owner and therefore bypasses RLS for its own writes while
-- still re-checking has_permission()/tenant ownership itself.
...
-- Creates a ticket and its first message atomically so a tenant never ends
-- up with an empty ticket if the message insert were to fail separately.
...
-- Dual-purpose read-receipt RPC: a tenant caller marks the support team's
-- messages read; a platform-admin caller marks the tenant's messages read.
-- Callable directly by either role's own JWT (no service-role needed).
...
-- Attachments: private bucket, tenant-folder-scoped (mirrors
-- hotel-expense-receipts / catalog-images). Super-admin reads resolve signed
-- URLs server-side via the service-role client (bypasses these policies
-- entirely), so no platform_admins branch is needed here.
...
-- Permissions catalog. `has_permission()` already gives every tenant's
-- 'Administrateur' role an unconditional bypass; the explicit grant below
-- just keeps the permissions-management UI's listing accurate (mirrors
-- 20260806100000_add_clients_export_permission.sql). Other roles only gain
-- access once a tenant admin grants it explicitly in that same UI.
