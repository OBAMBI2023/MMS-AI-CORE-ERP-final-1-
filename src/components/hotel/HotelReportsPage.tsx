  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const queries = [
        ...
        db.from("depenses").select("id,tenant_id,amount,paid_at").eq("tenant_id", tenantId),
      ];
...
function buildReport(data: any, range: Range, days: string[]) {
  ...
  return { hasData: periodReservations.length + payments.length + expensesRows.length > 0, paid, expenses, net: paid-expenses, occupancy: availableNights ? nights/availableNights*100 : 0, reservationCount:periodReservations.length, nights, due, averageBasket:periodReservations.length ? paid/periodReservations.length : 0, timeline, roomPerformance, roomRevenue:roomPerformance.filter((r:any)=>r.revenue>0).map((r:any)=>({name:r.name,value:r.revenue})), statusDistribution:group(periodReservations,(r)=>statusLabels[r.status] ?? r.status,()=>1), paymentDistribution:group(payments,(p)=>p.method || "Non renseigné",(p)=>Number(p.amount)), recentPayments, range, periodIds, roomMap };
}
