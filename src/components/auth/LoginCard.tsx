...
export function LoginCard({
  ...
  passwordId, emailPlaceholder, logo = PLATFORM_BRANDING.assets.logoVertical,
  ...
  if (premium) {
    return (
      <Card className="w-full rounded-[28px] border border-white/70 bg-white/[0.96] shadow-[0_32px_90px_rgba(0,8,35,0.38)] backdrop-blur-xl">
        <CardHeader className="space-y-0 px-6 pb-5 pt-7 text-center sm:px-10 sm:pt-9">
          ...
          <CardTitle className="text-[28px] font-semibold tracking-[-0.025em] text-slate-950">{title}</CardTitle>
        ...
        <CardContent className="px-6 pb-7 sm:px-10 sm:pb-9">
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor={emailId} className="text-sm font-medium text-slate-700">Adresse e-mail</Label>
              <div className="group relative">
                <Mail className="absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-400 transition-colors duration-200 group-focus-within:text-[#0F5BFF]" />
                <Input {...emailRegistration} id={emailId} type="email" autoComplete="email" placeholder={emailPlaceholder} className="h-13 rounded-2xl border-slate-200 bg-slate-50/70 pl-12 text-sm shadow-sm shadow-slate-100 transition-all duration-200 placeholder:text-slate-400 hover:border-slate-300 focus-visible:border-[#0F5BFF] focus-visible:bg-white focus-visible:ring-4 focus-visible:ring-blue-500/10" disabled={inputsDisabled} />
              </div>
              {emailError && <p className="text-xs font-medium text-red-600">{emailError.message}</p>}
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-4">
                <Label htmlFor={passwordId} className="text-sm font-medium text-slate-700">Mot de passe</Label>
                <button type="button" className="text-xs font-semibold text-[#0F5BFF] transition-colors hover:text-blue-700 hover:underline focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30">
                  Mot de passe oublié ?
                </button>
              </div>
              <div className="group relative">
                <Lock className="absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-400 transition-colors duration-200 group-focus-within:text-[#0F5BFF]" />
                <Input {...passwordRegistration} id={passwordId} type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="••••••••" className="h-13 rounded-2xl border-slate-200 bg-slate-50/70 pl-12 pr-12 text-sm shadow-sm shadow-slate-100 transition-all duration-200 placeholder:text-slate-400 hover:border-slate-300 focus-visible:border-[#0F5BFF] focus-visible:bg-white focus-visible:ring-4 focus-visible:ring-blue-500/10" disabled={inputsDisabled} />
                <button type="button" onClick={onTogglePassword} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-md p-1 text-slate-400 transition-colors hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30" disabled={inputsDisabled} aria-label={showPassword ? "Masquer le mot de passe" : "Afficher le mot de passe"}>
                  {showPassword ? <EyeOff className="h-[18px] w-[18px]" /> : <Eye className="h-[18px] w-[18px]" />}
                </button>
              </div>
              {passwordError && <p className="text-xs font-medium text-red-600">{passwordError.message}</p>}
            </div>
