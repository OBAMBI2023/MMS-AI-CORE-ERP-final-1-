...
import { TRIAL_ACTIVITIES } from "@/lib/public-trial-policy";
...
function TrialSignupPage() {
  ...
  const [form,setForm]=useState({companyName:"",adminName:"",activity:"Commerce",email:"",phone:"",password:"",confirmation:""});
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);try{
    const result=await createTrialWorkspace({data:{...form,activity:form.activity as (typeof TRIAL_ACTIVITIES)[number],turnstileToken:token}});
    await supabase.auth.setSession({access_token:result.accessToken,refresh_token:result.refreshToken});
    await navigate({to:"/demande-en-attente",replace:true});
  ...
  return <main className="min-h-screen bg-muted/30 px-4 py-12"><Card className="mx-auto max-w-2xl"><CardHeader><CardTitle>Créer votre espace d’essai</CardTitle><p className="text-sm text-muted-foreground">Votre espace sera configuré par notre équipe avant que l’essai ne démarre.</p></CardHeader><CardContent><form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="space-y-2 sm:col-span-2"><Label>Activité</Label><Select value={form.activity} onValueChange={activity=>setForm(v=>({...v,activity}))}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{TRIAL_ACTIVITIES.map(a=><SelectItem key={a} value={a}>{a}</SelectItem>)}</SelectContent></Select></div>
    ...
    <Button className="sm:col-span-2" disabled={busy||!token}>{busy?"Création…":"Envoyer ma demande"}</Button>
